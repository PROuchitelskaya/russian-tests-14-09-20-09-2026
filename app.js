...
(function () {
  ...
  var SITE_URL = "https://prouchitelskaya.github.io/russian-tests-14-09-20-09-2026/";
  ...
  var state = {
    ...
    selected: [],     // выбранные варианты в вопросе с множественным выбором
  ...
  function startQuiz(gradeId) {
    ...
    state.score = 0;
    state.locked = false;
    state.optOrders = grade.questions.map(function (q) {
  ...
  /* Вопрос с несколькими верными ответами: { "type": "multi", "answers": [0,1] }.
     Обычный вопрос: { "answer": 0 }. Верные варианты в data.js стоят первыми,
     на экране порядок перемешивается. */
  ...
  function renderQuestion() {
    ...
    $("q-hint").classList.toggle("hidden", !multi);
    ...
    order.forEach(function (origIdx, pos) {
      ...
      b.type = "button";
      b.className = "opt" + (multi ? " opt-multi" : "");
      b.dataset.orig = origIdx;
      b.innerHTML = '<span class="opt-letter">' + LETTERS[pos] + "</span><span>" + escapeHtml(q.options[origIdx]) + "</span>";
      if (multi) b.addEventListener("click", function () { toggleOpt(b, origIdx); });
      else b.addEventListener("click", function () { choose(b, origIdx); });
      box.appendChild(b);
    });
    ...
    var chk = $("btn-check");
    chk.classList.toggle("hidden", !multi);
    chk.disabled = true;
  }

  /* ---------- множественный выбор ---------- */
  function toggleOpt(btn, origIdx) {
    if (state.locked) return;
    var i = state.selected.indexOf(origIdx);
    if (i === -1) { state.selected.push(origIdx); btn.classList.add("sel"); }
    else { state.selected.splice(i, 1); btn.classList.remove("sel"); }
    $("btn-check").disabled = state.selected.length === 0;
  }
  ...
  function checkMulti() {
    ...
    var corr = q.answers.slice().sort(num);
    var sel = state.selected.slice().sort(num);
    // балл только при ТОЧНОМ совпадении набора: все верные и ни одного лишнего
    var ok = corr.length === sel.length && corr.every(function (v, i) { return v === sel[i]; });
    if (ok) state.score++;

    $("opts").querySelectorAll(".opt").forEach(function (b) {
      b.disabled = true;
      var orig = parseInt(b.dataset.orig, 10);
      if (q.answers.indexOf(orig) !== -1) b.classList.add("good");
      else if (state.selected.indexOf(orig) !== -1) b.classList.add("bad");
  ...
  function revealExplain(ok) {
    var q = state.grade.questions[state.idx];
    var ex = $("explain");
    ex.classList.remove("hidden");
    ex.classList.toggle("correct", ok);
    $("explain-verdict").textContent = ok ? "Верно!" : "Не совсем.";
  ...
  $("btn-check").addEventListener("click", checkMulti);
